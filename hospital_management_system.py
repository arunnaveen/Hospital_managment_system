...
#connection to mysql
con = sqlcon.connect(host = "127.0.0.1",user = 'root',password = "naveen12")
...
def entry():
    ...
    p5 = e5.get()
    p6 = e6.get()
    query="insert into appoinment values('{}','{}','{}','{}','{}','{}')".format(p1,p2,p3,p4,p5,p6)
    con.commit()
    cur.execute(query)
...
def apo_details():
    ...
    if int(p1) == 1:
        ...
        u = (23,34,12,67,53,72)
        o = rd.choice(u)
        det = ("Your appoinment is fixed with",h,
               "\nDate:-",p2,
               "\nTime:-",p3,
               "\nAppoinment No:-",o)
        query = 'insert into appointment_details values("{}", "{}", "{}", "{}", "{}")'.format(p1,h,p2,p3,o)
        cur.execute(query)
        tkinter.messagebox.showinfo("Appoinment Details", det)
...
def get_apoint():
    ...
    if len(a) == 0:
        ...
        root3 = Tk()
        label = Label(root3,text="APPOINMENT" , font='arial 25 bold')
        label.pack()
        ...
        L.place(x=50,y=220)
        L1 = Label(root3,text='1.Orthopeadic surgeon')
        L1.place(x=50,y=250)
        ...
        L4.place(x=50 , y=310)
        L5 = Label(root3 , text="5.Gyneacologist")
        L5.place(x=50,y=330)
        ...
        L8 = Label(root3 , text=('enter time in 24 hour formate')).place(x=48 , y=430)
...
def apoint():
    global x1
    root2 = Tk()
    label = Label(root2 , text="APPOINMENT",font='arial 25 bold')
    label.pack()
...
def ser_avail():
    ...
    frame.pack()
    l1 = Label(root5,text='SERVICE AVAILABLE')
    l1.place(x=20,y=10)
...
def modify():
    ...
    if len(a) == 0:
        ...
        L1.place(x=50 , y=50)
        L2 = Label(root6 , text='ENTER NEW DETAILS')
        L2.place(x=50 , y=360)
...
def do_modify():
        ...
        if choice == '1':
            cur.execute('update appoinment set name={} where idno={}'.format(new,ad))
        elif choice == '2':
...
def view_data():
    global p1
    p1 = x3.get()
    cur.execute('select * from appoinment where idno=(%s)',(p1,))
    ...
    for i in dat:
        a.append(i)
    if len(a) == 0:
        tkinter.messagebox.showwarning("DATA",'NO DATA FOUND!!')
    else:
        det = a
        tkinter.messagebox.showinfo("APPOIMMENT DETAILS",det)
